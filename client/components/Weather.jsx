...
const Weather = () => {
    ...
    return (
      <div className='weather-forecast'>
        ...
        {weatherData.time.map((data, index) => (
          <div key={index}>
            <p className='day-of-week-date'>
              <FontAwesomeIcon icon={faCalendarDay} size='2x' beat /> :{' '}
              {daysOfWeek[dayjs(weatherData.time[index]).format('d')]}{' '}
              {dayjs(weatherData.time[index]).format('D MMM YYYY')}
            </p>
            ...
            <p className='high-temp'>
              <FontAwesomeIcon icon={faTemperatureHigh} size='2x' beat /> :{' '}
              {weatherData.temperature_2m_max[index]}°F
            </p>
            <p className='low-temp'>
              <FontAwesomeIcon icon={faTemperatureLow} size='2x' beat /> :{' '}
              {weatherData.temperature_2m_min[index]}°F
            </p>
            <p className='sunrise'>
              <FontAwesomeIcon icon={faSun} size='2x' beat /> :{' '}
              {dayjs(weatherData.sunrise[index]).format(' h:mm A')}
            </p>
            <p className='sunset'>
              <FontAwesomeIcon icon={faCloudSun} size='2x' beat /> :{' '}
              {dayjs(weatherData.sunset[index]).format(' h:mm A')}
            </p>
          </div>
        ))}
      </div>
  ...
  return (
    <div align='center'>
      <br></br>
      <h3>You must be logged in to view the weather</h3>
      <img
        width='200'
        height='100%'
        src='https://media2.giphy.com/media/26BRGFziXdKCxxDJS/giphy.gif?cid=ecf05e471nj42aiw9ogdqdy36on95j0s5qvtkfpw5aknyuh2&rid=giphy.gif&ct=s'
      />
    </div>
  );
};
